...
<?php
echo "<form action=\"update_house2.php\" method=post>";

	$conn = mysql_connect("cronus.cs.uleth.ca","tymj3660","mkirch") or die(mysql_error());
	mysql_select_db("tymj3660",$conn); 

	$sql = "select * from HOUSES where PostalCode='$_POST[postalCode]'"; 
	$result = mysql_query($sql,$conn);
    if(mysql_num_rows($result) != 0)
	{
	   $rec=mysql_fetch_row($result); 
	   
	   echo "Postal Code: <input type=text  name=\"postalCode\" value=\"$rec[0]\"><br><br>";
	   echo "Location: <input type=text  name=\"location\" value=\"$rec[1]\"><br><br>";
	   echo "Agent ID: <input type=text  name=\"agentID\" value=\"$rec[2]\"><br><br>";
	   echo "Size: <input type=text  name=\"size\" value=\"$rec[3]\"><br><br>";
	   echo "Price: <input type=text  name=\"price\" value=\"$rec[4]\"><br><br>";
	   echo "Type: <input type=text  name=\"type\" value=\"$rec[5]\"><br><br>";
	   echo "Facilities: <input type=text  name=\"facilities\" value=\"$rec[6]\"><br><br>";
	   echo "<input type=hidden name=\"oldPostalCode\" value=\"$_POST[postalCode]\">";
