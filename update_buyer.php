...
<?php
echo "<form action=\"update_buyer2.php\" method=post>";

	$conn = mysql_connect("cronus.cs.uleth.ca","tymj3660","mkirch") or die(mysql_error());
	mysql_select_db("tymj3660",$conn); 

	$sql = "select * from BUYER where name='$_POST[ID]'"; 
	$result = mysql_query($sql,$conn);
    if(mysql_num_rows($result) != 0)
	{
	   $rec=mysql_fetch_row($result); 
	   echo "Buyer ID: <input type=text name=\"ID\" value=\"$rec[0]\"><br><br>";
	   echo "Buyer Name: <input type=text name=\"buyerName\" value=\"$rec[1]\"><br><br>";
