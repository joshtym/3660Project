...
<?php
echo "<form action=\"update_agent2.php\" method=post>";

	$conn = mysql_connect("cronus.cs.uleth.ca","tymj3660","mkirch") or die(mysql_error());
	mysql_select_db("tymj3660",$conn); 

	$sql = "select * from AGENT where ID='$_POST[ID]'"; 
	$result = mysql_query($sql,$conn);
    if(mysql_num_rows($result) != 0)
	{
	   $rec=mysql_fetch_row($result); 
	   echo "Agent ID: <input type=text name=\"ID\" value=\"$rec[0]\"><br><br>";
	   echo "Agent Name: <input type=text name=\"agentName\" value=\"$rec[1]\"><br><br>";
	...
	{
		echo "No data found within the table</p>"; 
	}
